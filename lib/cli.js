...
class CLI {
  ...
  runOption6() { 
    ...
    const sql2 = `SELECT employees.id, CONCAT(employees.first_name, ' ', employees.last_name) AS manager
    FROM employees
    ...
    db.query(sql2, (err, results) => {
      ...
      for (let k=0; k < results.length; k++) {
        if (k == 0) {
          emIdArray[0] = results[0].id;
        } else {
          emIdArray.push(results[k].id);
        }
      }
      inquirer
      .prompt([
      ...
      .then(({ add_firstName, add_lastName, roleTable, managerTable }) => {
        ...
        if (managerTable == "NULL") {
          ...
          j = emIdArray[managerArray.indexOf(managerTable)-1];
        }
  ...
  runOption7() { 
    ...
    db.query(sql3, (err, results) => {
      ...
      .then(({ emTable, roleTable }) => {
        ...
        m = parseInt(managerArray[j-1]);
        if (m == NaN) {
          m = null;
        }
        const sql4 = `UPDATE employees
        SET employees.role_id = ${j}, manager_id = ${m}
